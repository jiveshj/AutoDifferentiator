...
class VariableAdd(Operation):
    ...
    def forward_call(self, summands):
        ...
        add_data = np.sum([summand.data for summand in summands], axis = 0)
        return add_data        
    ...
    def backward_call(self, downstream_grad):
        ...
        ### YOUR CODE HERE ###
        gradients = [downstream_grad for _ in self.parents]
        return gradients
...
class VariableMultiply(Operation):
    ...
    def forward_call(self, multiplicands):
        ...
        multiply_answer = multiplicands[0].data
        for multiplicand in multiplicands[1:]:
            multiply_answer = np.multiply(multiply_answer, multiplicand.data)
    ...
    def backward_call(self, downstream_grad):
        ...
        for i in range(num_parents):
            grad = downstream_grad

            for j in range(num_parents):
                if i != j:
                    grad = np.multiply(grad, self.parents[j].data)
...
class ScalarMultiply(Operation):
    ...
    def forward_call(self, scalar, tensor):
        ...
        result = scalar.data * tensor.data
    ...
    def backward_call(self, downstream_grad):
        ...
        grad_wrt_scalar = np.sum(downstream_grad * tensor.data)

        grad_wrt_tensor = scalar.data * downstream_grad

        return [grad_wrt_scalar, grad_wrt_tensor]
...
class MatrixMultiply(Operation):
    ...
    def backward_call(self, downstream_grad):
        ...
        ### YOUR CODE HERE ###
        A,B = self.parents
        grad_A = np.dot(downstream_grad, B.data.T)

        grad_B = np.dot(A.data.T, downstream_grad)
...
class HingeLoss(Operation):
    ...
    def forward_call(self, scores):
        ...
        correct_class_score = scores.data[self.label]

        loss = 0
        for i in range(scores.data.shape[0]):
            if i != self.label:
                margin = 1 + scores.data[i] - correct_class_score
                loss += max(0, margin)

        loss /= scores.data.shape[0]
    ...
    def backward_call(self, downstream_grad):
        ...
        ### YOUR CODE HERE ###
        scores = self.parents[0]
        grad = np.zeros_like(scores.data)
        ...
        for i in range(scores.data.shape[0]):
            if i != self.label:
                margin = 1 + scores.data[i] - correct_class_score
                if margin > 0:
                    grad[i] = downstream_grad 
                    grad[self.label] -= downstream_grad  
        grad /= scores.data.shape[0]
